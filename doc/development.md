...
Attach the resulting `AppEEARS.zip` to the GitHub Release for that tag; this is the file linked from the user guide's install instructions. Before publishing, sanity-check it by installing it into a fresh QGIS profile via `Install from ZIP` to confirm nothing needed at runtime was left out of the archive.